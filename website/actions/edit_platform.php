...
try
{
	...
	$Platform = $API->GetPlatforms($_REQUEST['id']);
	if(isset($Platform[$_REQUEST['id']]))
	{
		$Platform = $Platform[$_REQUEST['id']];
		$API->updatePlatform($Platform->id, $_REQUEST['name'], $_REQUEST['developer'], $_REQUEST['manufacturer'], $_REQUEST['media'], $_REQUEST['cpu'], $_REQUEST['memory'], $_REQUEST['graphics'], $_REQUEST['sound'], $_REQUEST['maxcontrollers'], $_REQUEST['display'], $_REQUEST['overview'], $_REQUEST['youtube']);
		$id = $Platform->id;
		...
		returnJSONAndDie(0, "Platform Updated." . $error_msg, $id, $name);
	}
	else
	{
